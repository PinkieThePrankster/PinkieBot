...
hello.rule = r'(?i)(hi|hello|hey) (?i)$nickname\b'
...
sniff.rule = r'(?i)(\x01ACTION sniffs (?i)$nickname)'
...
grope.rule = r'(?i)(\x01ACTION runs over and squeezes (?i)$nickname)'
...
flirt.rule = r'(?i)(\x01ACTION winks at (?i)$nickname)'
...
lick.rule = r'(?i)(\x01ACTION licks (?i)$nickname)'
...
growl.rule = r'(?i)(\x01ACTION growls at (?i)$nickname)'
...
kiss.rule = r'(?i)(\x01ACTION kisses (?i)$nickname)'

def interjection(phenny, input): 
    phenny.say(input.nick + '!')
interjection.rule = r'(?i)$nickname!'
...
hugs.rule = r'(\x01ACTION)? (?i)(hugs) (?i)$nickname'
...
tickles.rule = r'(\x01ACTION)? (?i)(tickles) (?i)$nickname'
...
parties.rule = r'(\x01ACTION)? (?i)(parties with) (?i)$nickname'
...
thanks.rule = r'(?i)(Thank( you|s)?(,)? (?i)$nickname)'
...
pokes.rule = r'(\x01ACTION)? (?i)pokes (?i)$nickname'
...
bestpony.rule = r'(?i)($nickname(:|,)? )?(who(\'s|s| is)) (the )?best (pony|robomare|mare)[\?]?'
...
boops.rule = r'(\x01ACTION)?( "accidentally"| trips and accidentally| sticks her tongue out and| gently| tries to)? (?i)boop(s)? (?i)$nickname'
